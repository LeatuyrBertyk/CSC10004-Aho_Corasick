...
// 1. Auxiliary structure to solve the Maximal Munch problem (Preventing
// overlapping matches) start is the starting index, end is the index of the
// last character (Inclusive). Example: If the pattern 'abc' appears at the
// beginning of the string (index 0), then start=0, end=2.
struct Match {
  int start;
  int end;
};
...
struct TrieNode {
  ...
  int word_length;     // The length of the phrase (used to mask with '*'
                       // characters)
